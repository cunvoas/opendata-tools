...
public class ParksPropositionController {
     ...
     */
    @PostMapping("/delete")
    public String delete(@ModelAttribute FormProposition form, Model model) {
    	
    	
        log.info("delete() - form={}, commune={}", form, form.getCodeInsee());
        
//        if (form.getId() == null ) {
//            log.warn("Code manquant lors de l'effacement de la proposition");
//            model.addAttribute("delete", "Erreur: Aucune proposition sélectionnée");
//            return show(form, model);
//        }
        
        try {
        	servicePropositionParc.deleteParkProposal(form.getId());
        
        } catch (Exception e) {
            log.error("Erreur lors de l'effacement des propositions", e);
            model.addAttribute("delete", "Erreur lors de l'effacement: " + e.getMessage());
        }
