...
/**
 * Analyseur de fichiers CSV pour les parcs et jardins de Roubaix.
 ...
 */
@Component
public class CsvRbxParkJardinParser {
		...
		if (csvFile.isFile()) {
			...
			try (Reader reader = new FileReader(csvFile)) {
				...
				for (CSVRecord row : rows) {
					...
					park.setType("Parc");
					String s = row.get(ParkJardinCsvHeaders.surface);
					if (!"".equals(s)) {
						park.setSurface(Double.parseDouble(row.get(ParkJardinCsvHeaders.surface))*10000);
					}
